...
public class PetProvider extends ContentProvider {
    ...
    private Uri insertPet(Uri uri, ContentValues values) {
        ...
        String name = values.getAsString(PetContract.PetEntry.COLUMNS_PET_NAME);
        if (name == null) {
            throw new IllegalArgumentException("Pet requires a name");
        }

        Integer gender = values.getAsInteger(PetContract.PetEntry.COLUMNS_PET_GENDER);
        if (gender == null || !PetContract.PetEntry.isValidGender(gender)) {
            throw new IllegalArgumentException("Pet must have a valid gender");
        }

        // If the weight is provided, check that it's greater than or equal to 0 kg
        Integer weight = values.getAsInteger(PetContract.PetEntry.COLUMNS_PET_WEIGHT);
    ...
    private int updatePet(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(PetContract.PetEntry.COLUMNS_PET_NAME)) {
            String name = values.getAsString(PetContract.PetEntry.COLUMNS_PET_NAME);
            if (name == null) {
                throw new IllegalArgumentException("Pet requires a name");
            }
        ...
        if (values.containsKey(PetContract.PetEntry.COLUMNS_PET_GENDER)) {
            Integer gender = values.getAsInteger(PetContract.PetEntry.COLUMNS_PET_GENDER);
            if (gender == null || !PetContract.PetEntry.isValidGender(gender)) {
                throw new IllegalArgumentException("Pet must have a valid gender");
            }
        ...
        if (values.containsKey(PetContract.PetEntry.COLUMNS_PET_WEIGHT)) {
            Integer weight = values.getAsInteger(PetContract.PetEntry.COLUMNS_PET_WEIGHT);
            if (weight != null && weight < 0) {
                throw new IllegalArgumentException("Pet requires valid weight");
            }
